...
def main(_):
  ...
  word_vector_list = list()
  for result in estimator.predict(input_fn, yield_single_examples=True):
      unique_id = int(result["unique_id"])
      feature = unique_id_to_feature[unique_id]
      temp = np.array([feature.input_mask],dtype=float)
      temp1 = np.array(result['layer_output_0'])
      word_vector_list.append(temp.T*temp1)
  sample = np.array(word_vector_list)
  (sample_size,time_steps,input_dim) = sample.shape
  model = Sequential()
  model.add(LSTM(output_dim=1,
                    input_shape=(time_steps,input_dim),
                    return_sequences=True))
  model.compile(optimizer="adam",loss="mse")
  y = np.zeros(shape=(sample_size,time_steps,1))
  model.fit(sample,y,epochs=5,batch_size=sample_size)
